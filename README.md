...
So what did I learn overall from this.  Gemini 3.0 like all other models do really well building things from the ground up.  Whne you try to pivot existing code you have to be very careful in how you context and architect the intial prompts.  In the case of redveloping an app you and the llm have to go program by program in the app to map out what it does and why.  Then you can start the process of redeveloping the app to the new purpose.  LLMs are not one shot generators when it comes to repurposing existing code.  This is important for anyone who wants to do something like I did.  Build an app from scratch like the drone one then try to pivot the same process to something different like quadrupeds.  
